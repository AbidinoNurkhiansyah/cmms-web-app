<x-modal wire:model="formModal" title="{{ $editId ? 'Edit Spare Part' : 'Add New Spare Part' }}"
    box-class="w-11/12 max-w-5xl max-h-[95vh]" separator>
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        {{-- Form Column --}}
        <div class="md:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2">
            <x-input label="Part Name *" wire:model="part_name" class="md:col-span-3" />
            
            <x-input label="Part Number" wire:model="part_number" />
            <x-input label="Group" wire:model="group" />
            <x-select label="Rank" wire:model="rank" :options="[['id' => 'A', 'name' => 'A'], ['id' => 'B', 'name' => 'B'], ['id' => 'C', 'name' => 'C'], ['id' => 'D', 'name' => 'D']]" placeholder="Select Rank" />
            ...
            <x-input label="Price (IDR) *" type="number" wire:model="price_idr" prefix="Rp" class="md:col-span-2" />
            
            <x-select label="Status *" :options="[['id' => 'Y', 'name' => 'Active'], ['id' => 'N', 'name' => 'Discontinued']]" wire:model="status" />
            <div class="md:col-span-2">
                <x-choices label="Machine(s)" wire:model="machine" :options="$assets" multiple searchable search-function="searchMachines" />
            </div>
        </div>
    </div>
