<div class="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
    ...
    <div class="w-full md:w-1/3">
        <x-input icon="o-magnifying-glass" wire:model.live.debounce.500ms="search" placeholder="Cari lokasi, bahaya, atau user..."
            clearable />
    </div>
...
<x-card>
    <x-table :headers="$this->headers" :rows="$this->records" with-pagination>
        @scope('cell_date', $kyt)
        {{ \Carbon\Carbon::parse($kyt->date)->format('d-M-Y') }}
        @endscope

        @scope('cell_user.name', $kyt)
        {{ $kyt->user->name ?? '-' }}
        @endscope


        @scope('cell_bahaya', $kyt)
        {{ Str::limit($kyt->bahaya, 30) }}
        @endscope

        @scope('cell_actions', $kyt)
        <div class="flex flex-nowrap gap-1 justify-center">
            <x-button icon="o-eye" wire:click="showDetail({{ $kyt->no }})" class="btn-sm btn-ghost text-green-500" spinner />
            <x-button icon="o-pencil" wire:click="edit({{ $kyt->no }})" class="btn-sm btn-ghost text-blue-500" spinner />
            <x-button icon="o-trash" wire:click="confirmDelete({{ $kyt->no }})" class="btn-sm btn-ghost text-error"
                spinner />
        </div>
        @endscope
    </x-table>
    ...
    @if($this->records->isEmpty())
        <div class="text-center py-8 text-base-content/60">
            ...
            <p>Tidak ada data KYT ditemukan untuk bulan
                {{ \Carbon\Carbon::parse($currentMonth . '-01')->translatedFormat('F Y') }}
...
<x-modal wire:model="confirmModal" title="Konfirmasi Hapus" class="backdrop-blur-sm">
    <div class="py-4 text-base-content/80 text-center">
        ...
        <p class="text-lg">Apakah Anda yakin ingin menghapus data KYT ini?</p>
        <p class="text-sm opacity-70 mt-1">Tindakan ini tidak dapat dibatalkan dan foto terkait akan dihapus.</p>
    </div>
    <x-slot:actions>
